...
> mitad de la app.
>
> _Última actualización: 2026-08-25 (Fase 21 + catálogo de insulinas)._
...
| Qué más se registró durante un episodio | `MealEpisodeMetrics.contextEvents` (schemas) ← `collectEpisodeContext` (domain) + `getEventsDuringEpisode` (db) | 2026-08-22 ✅ (Fase 23). El chat puede decir "durante ese seguimiento también registraste una corrección de 2 U a las 2 h" — y **nunca** si correspondía, si hacía falta, o qué hacer distinto. Dos cosas que el chat debe heredar tal cual: (a) **no hay campo de texto** — el texto de una nota nunca sale del teléfono, y la garantía es que el tipo no tiene dónde ponerlo, no una frase del prompt; (b) **ausente significa "no se capturó", nunca "no pasó nada"** — un episodio viejo no tiene el campo, y afirmar que fue tranquilo sería inventar. **(c) agregado el 2026-08-22 tras la revisión de seguridad: con la lista de dosis a la vista, el modelo puede afirmar superposición ("la segunda se solapó con la primera, que seguía activa") sin recomendar nada — y eso es una estimación de IOB, prohibida por `AGENTS.md` igual que recomendar una dosis. El chat hereda la prohibición explícita del prompt (`glucose-insight.v4`) y los patrones de IOB que se le agregaron a `containsTherapyRecommendation`. Regla general para la fase del chat: cada vez que se le da al modelo un dato nuevo, revisar si el filtro de salida cubre lo que ese dato le permite decir.** |
...
| Qué insulina usa y cuánto dura | `TherapyProfile.rapidInsulinId/basalInsulinId` + `*DurationHours`; catálogo en `INSULIN_CATALOG` (domain) | 2026-08-25 ✅ (R). El chat puede decir cuál eligió y qué duración tiene guardada. **Nunca puede usarla para estimar insulina activa, superposición de dosis, ni cuánto queda actuando** — eso es IOB, prohibido en el MVP por `AGENTS.md`, y el filtro de salida ya tiene patrones para esa clase de frase (ver `containsTherapyRecommendation`). Su único uso legítimo es explicar por qué un tramo quedó fuera de un promedio. Si no hay ninguna elegida, decir eso, nunca suponer una. |
| Cuán confundido está un patrón | `MacroGlucosePoint.adjusted` / `.confoundedCount`, `HorizonOutcome.confoundedCount` (domain) | 2026-08-26 ✅ (R). El chat **nunca** puede citar un promedio de Patrones sin decir de cuántas comidas sale y cuántas traían otra comida, dosis o actividad de por medio — y si está ajustado por covariables o es el crudo. Un promedio ajustado y uno crudo no significan lo mismo. Tampoco puede presentar el ajuste como si limpiara el dato: reduce el sesgo de un confusor medido, no lo elimina. |
| ⛔ `packages/domain/src/regression.ts` | — | **NO alcanzable por el chat, a propósito.** Es OLS genérico. El coeficiente de una columna de unidades de insulina es, dimensionalmente, un factor de corrección empírico derivado de los datos de la usuaria, y `AGENTS.md` prohíbe inferir parámetros de terapia. Su único llamador legítimo es `macro-glucose.ts`, que residualiza y descarta los coeficientes. No exponerlo como herramienta. |
...
| Grasa/proteína frente a la glucosa tardía (2/3/4/5 h) | `buildMacroGlucoseComparison` (domain, `macro-glucose.ts`) | 2026-08-20 ✅, **corregida 2026-08-22 (Fase 23)**: ahora excluye por horizonte los episodios con un evento confundente en el medio — hasta entonces una colación a las 2 h se promediaba como efecto tardío de la grasa. Si el chat alguna vez llama a esta función, **tiene que pasarle `insulin`, `carbs` y `activity`**: son opcionales por compatibilidad, y omitirlos la devuelve en silencio a la versión que mezclaba. **La capacidad más peligrosa del catálogo.** Describe una subida retrasada y prolongada que la literatura resuelve **ajustando la insulina** (bolo dual o extendido) — el chat **nunca** puede sugerir eso, ni un tiempo de espera, ni comer menos grasa o proteína. Puede decir qué pasó con sus datos y que lo converse con su equipo. Arrastrar siempre los `sampleSize` y respetar `meanDeltaMgDl: undefined` como "sin datos suficientes", nunca como 0. Toda salida pasa por `containsTherapyRecommendation`. |
