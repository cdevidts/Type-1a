...
| **3** | ✅ **Gráfico rediseñado (2026-08-17), pendiente confirmar en dispositivo.** `GlucoseChart.tsx`: ahora marca cada punto (no solo el último), con color según rango (rojo &lt;70, naranjo &gt;180, teal en rango); ejes Y con valores numéricos (70/180) junto a las líneas guía; leyenda inferior con 3 horas reales (inicio/medio/ahora) en vez de texto fijo "-3h"/"Ahora". Pendiente: `getTimeline()` (a) y tocar ítems (b) — ver abajo. **Agregado tras probar la Fase 2:** (a) ✅ fix chico ya aplicado — `getTimeline()` ahora incluye `cgm_readings` (kind `'glucose'`, tono teal, sufijo "· importado" cuando corresponde); pendiente para la fase completa: los `synthetic` no se distinguen visualmente en el Timeline (sí en el resto de la app — GlucoseCard/SettingsModal), y la mezcla de las 5 queries con `LIMIT` + slice puede dejar que la densidad de CGM saque de vista eventos de insulina/comida una vez que haya varios días de historial real — hay que resolverlo antes de confiar en el Timeline como "ya registré esto"; (b) ítems del Timeline deben ser tocables (`Pressable`) con vista de detalle como mínimo — edición completa puede quedar para una fase propia si el detalle solo resulta insuficiente. | — (independiente, se puede hacer en paralelo a 1-2) |
