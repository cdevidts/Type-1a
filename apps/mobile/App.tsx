...
function Type1AApp() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      // Saca las fotos de la caché de Android, que las borra sin avisar. Va
      // acá y no en `refresh()` porque es de una sola vez por foto: cuando ya
      // están todas movidas no encuentra trabajo y sale enseguida.
      try {
        await migrateCachedPhotos(db);
      } catch (error) {
        // Nunca puede impedir que la app abra: sin esto la foto sigue en la
        // caché, que es exactamente como estaba antes de este arreglo.
        logSaveError('migrateCachedPhotos', error);
      }
