...
packages/cgm/      CGMProvider + implementaciones.  packages/ai/  Abacus RouteLLM.
...
| `agent-context.ts` | qué se le manda al modelo. **Los parámetros de terapia NO viajan**: con ratio y factor, calcular una dosis es una división que el filtro de salida no notaría. `forbiddenKeysIn` lo afirma en un test |
| `agent-tools.ts` | qué alcanza el agente y qué **no, con su motivo**. `verify:contracts` falla si una función de `db.ts` no está en ninguna lista. Ver `docs/adr/0008` |
